...
//		LOG.info("getCpuScore -- " + score);
		return score >= BASESCORE ? BASESCORE : score;
	}

	public int getMemScore() {
		int score = (int) (memused * 100 / totalmem);
//		LOG.info("getMemScore -- " + memused + "*100/" + totalmem + "= " + score);
		return score >= BASESCORE ? BASESCORE : score;
		}

	public int getNetScore() {
		int score = (int) (((uploadNetBand + downloadNetBand) / bandwidth) * 100);
//		LOG.info("getNetScore -- ((" + uploadNetBand +"+" +downloadNetBand + ")/" + bandwidth + ") * 100 = " + score);	
		return score >= BASESCORE ? BASESCORE : score;
	}

	public int compareTo(ServerInfo si) {
//		int socre1 = getCpuScore() + getMemScore() + getNetScore();
//		int score2 = si.getCpuScore() + si.getMemScore() + si.getNetScore();
